...
Voter.GaugeCreated.handler(async ({ event, context }) => {
  ...
  if (poolEntity) {
    const poolUpdateDiff = {
      ...
      feeVotingRewardAddress: event.params.feeVotingReward.toLowerCase(),
      bribeVotingRewardAddress: event.params.bribeVotingReward.toLowerCase(),
...
Voter.DistributeReward.handler(async ({ event, context }) => {
  ...
  const result = await computeVoterDistributeValues({
    ...
    gaugeAddress: event.params.gauge.toLowerCase(),
    voterAddress: event.srcAddress.toLowerCase(),
