...
import POOL_FACTORY_ABI from "../../abis/PoolFactory.json";
...
/**
 * Fetches a token's USD price by reading pool reserves on-chain.
 * Used as a fallback on chains without a deployed SpotPriceAggregator oracle.
 *
 * Strategy:
 * 1. Look for a direct token-USDC pool and derive price from reserves
 * 2. If no direct pool, try 2-hop: token->WETH pool + WETH->USDC pool
 * 3. Return 0 if no price path exists
 *
 * Returns price in 18-decimal format (same as oracle).
 */
export async function fetchTokenPriceFromReserves(
  tokenAddress: string,
  usdcAddress: string,
  wethAddress: string,
  ...
  // Helper: get pool address from factory
  async function getPool(
    ...
    try {
      ...
      return (result as string) || ZERO_ADDRESS;
    } catch {
      return ZERO_ADDRESS;
    }
  }

  // Helper: get reserves and token0 from a pool
  async function getPoolReserves(poolAddress: string): Promise<{
    reserve0: bigint;
    reserve1: bigint;
    token0: string;
  } | null> {
  ...
  // Helper: compute price of tokenA in terms of tokenB using reserves
  // Returns price with 18 decimals
  function computePrice(
    reserves: { reserve0: bigint; reserve1: bigint; token0: string },
    tokenA: string,
    tokenADecimals: number,
    tokenBDecimals: number,
  ): bigint {
    const isToken0 =
      tokenA.toLowerCase() === reserves.token0.toLowerCase();
    ...
    // price = (reserveB * 10^tokenADecimals * 10^18) / (reserveA * 10^tokenBDecimals)
    return (
      (reserveB * 10n ** BigInt(tokenADecimals) * TEN_18) /
      (reserveA * 10n ** BigInt(tokenBDecimals))
    );
  }

  try {
    // Get token decimals
    const [tokenDecimalsResult, usdcDecimalsResult] = await Promise.all([
      ethClient.simulateContract({
    ...
    const tokenDecimals = Number(tokenDecimalsResult.result);
    const usdcDecimals = Number(usdcDecimalsResult.result);

    // 1. Try direct token-USDC pool (volatile)
    const directPoolAddr = await getPool(tokenAddress, usdcAddress, false);
    if (directPoolAddr !== ZERO_ADDRESS) {
      const reserves = await getPoolReserves(directPoolAddr);
      if (reserves && reserves.reserve0 > 0n && reserves.reserve1 > 0n) {
        ...
        if (price > 0n) {
          logger.info(
            `[fetchTokenPriceFromReserves] Direct USDC pool price for ${tokenAddress}: ${price}`,
          );
          return price;
        }
      }
    ...
    // 2. Try 2-hop: token -> WETH -> USDC
    if (wethAddress && wethAddress !== tokenAddress) {
      const [tokenWethPoolAddr, wethUsdcPoolAddr] = await Promise.all([
        getPool(tokenAddress, wethAddress, false),
        getPool(wethAddress, usdcAddress, false),
      ]);

      if (
        tokenWethPoolAddr !== ZERO_ADDRESS &&
        wethUsdcPoolAddr !== ZERO_ADDRESS
      ) {
        const [tokenWethReserves, wethUsdcReserves, wethDecimalsResult] =
          await Promise.all([
            getPoolReserves(tokenWethPoolAddr),
            getPoolReserves(wethUsdcPoolAddr),
            ethClient.simulateContract({
        ...
        const wethDecimals = Number(wethDecimalsResult.result);

        if (
          tokenWethReserves &&
          wethUsdcReserves &&
          tokenWethReserves.reserve0 > 0n &&
          tokenWethReserves.reserve1 > 0n &&
          wethUsdcReserves.reserve0 > 0n &&
          wethUsdcReserves.reserve1 > 0n
        ) {
          // tokenPriceInWeth (18 decimals)
          const tokenPriceInWeth = computePrice(
            tokenWethReserves,
            tokenAddress,
            tokenDecimals,
            wethDecimals,
          );
          // wethPriceInUsdc (18 decimals)
          const wethPriceInUsdc = computePrice(
            wethUsdcReserves,
            wethAddress,
            wethDecimals,
            usdcDecimals,
          ...
          if (tokenPriceInWeth > 0n && wethPriceInUsdc > 0n) {
            // Combine: (tokenPriceInWeth * wethPriceInUsdc) / 10^18
            const price = (tokenPriceInWeth * wethPriceInUsdc) / TEN_18;
            logger.info(
              `[fetchTokenPriceFromReserves] 2-hop WETH price for ${tokenAddress}: ${price}`,
            );
            return price;
          }
        }
    ...
    logger.info(
      `[fetchTokenPriceFromReserves] No price path found for ${tokenAddress} on chain ${chainId}`,
    );
    return 0n;
  } catch (error) {
    logger.error(
      `[fetchTokenPriceFromReserves] Error fetching price for ${tokenAddress} on chain ${chainId}: ${error}`,
    );
    return 0n;
  }
}
...
export function roundBlockToInterval(
  ...
  // Don't round to 0 - contracts don't exist at block 0
  return rounded > 0 ? rounded : blockNumber;
}
...
export const getTokenPrice = createEffect(
  ...
  async ({ input, context }) => {
    ...
    // Handle USDC special case (case-insensitive for RPC-sourced addresses)
    ...
    if (!ORACLE_DEPLOYED) {
      // Use reserve-based pricing fallback if pool factory is configured
      const poolFactoryAddress = CHAIN_CONSTANTS[chainId].poolFactoryAddress;
      if (poolFactoryAddress) {
        const ethClient = CHAIN_CONSTANTS[chainId].eth_client;
        try {
          const priceFromReserves = await fetchTokenPriceFromReserves(
            tokenAddress,
            USDC_ADDRESS,
            WETH_ADDRESS,
            poolFactoryAddress,
          ...
          return {
            pricePerUSDNew: priceFromReserves,
            priceOracleType: "reserve-based",
          };
        } catch (error) {
          context.log.error(
            `[getTokenPrice] Reserve-based pricing failed for ${tokenAddress} on chain ${chainId}: ${error}`,
          );
        }
      }
