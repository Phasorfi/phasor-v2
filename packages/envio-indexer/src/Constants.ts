...
export const VOTER_NONCL_POOLS_FACTORY_LIST: string[] = [
  ...
  ...(process.env.ENVIO_MONAD_POOL_FACTORY_ADDRESS
    ? [process.env.ENVIO_MONAD_POOL_FACTORY_ADDRESS]
    : []), // monad (dynamic)
].map((x) => toChecksumAddress(x));
...
export const MONAD_PRICE_CONNECTORS: PriceConnector[] =
  (PriceConnectors as Record<string, PriceConnector[]>).monad ?? [];
...
type chainConstants = {
  ...
  // Optional: PoolFactory address for reserve-based pricing on chains without an oracle
...
// Custom chain definition for Monad (not in viem/chains)
const monadLocal = {
  id: 10143,
  name: "Monad",
  ...
  rpcUrls: {
    default: {
      http: [process.env.ENVIO_MONAD_RPC_URL || DefaultRPC.monad],
    },
  },
} as const satisfies Chain;

// Constants for Monad (Phasor DEX)
const MONAD_CONSTANTS: chainConstants = {
  weth: process.env.ENVIO_MONAD_WMON_ADDRESS || "",
  usdc: process.env.ENVIO_MONAD_USDC_ADDRESS || "",
  oracle: {
    getType: (_blockNumber: number) => {
      return PriceOracleType.V3;
    },
    getAddress: (_priceOracleType: PriceOracleType) => {
      return ""; // No oracle deployed; reserve-based fallback used instead
    },
    startBlock: Number.MAX_SAFE_INTEGER, // No oracle deployed
    updateDelta: 60 * 60, // 1 hour
    priceConnectors: MONAD_PRICE_CONNECTORS,
  },
  rewardToken: (_blockNumber: number) =>
    process.env.ENVIO_MONAD_PHASOR_ADDRESS || "",
  ...
  lpHelperAddress: "",
  poolFactoryAddress: process.env.ENVIO_MONAD_POOL_FACTORY_ADDRESS || "",
};
 ...
 */
export const PoolId = (chainId: number, pool: string) =>
...
export const TokenIdByBlock = (
  ...
  chainId: number,
  blockNumber: number,
) => `${chainId}-${address.toLowerCase()}-${blockNumber}`;
