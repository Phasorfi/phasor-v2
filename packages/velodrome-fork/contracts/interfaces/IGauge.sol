...
interface IGauge {
    ...
    /// @notice Address of the token (PHASOR) rewarded to stakers
    ...
    /// @notice Minimum time multiplier (1x = 1e18)
    function TIME_MULTIPLIER_MIN() external view returns (uint256);

    /// @notice Maximum time multiplier (3x = 3e18)
    function TIME_MULTIPLIER_MAX() external view returns (uint256);

    /// @notice Period to reach maximum multiplier (90 days)
    function TIME_MULTIPLIER_PERIOD() external view returns (uint256);

    /// @notice Get the timestamp of first stake for an account
    function userFirstStakeTime(address) external view returns (uint256);
    ...
    function getTimeMultiplier(address _account) external view returns (uint256);
