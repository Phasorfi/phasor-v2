...
/// @title Phasor Gauge (Velodrome V2 Fork)
/// @author velodrome.finance, @figs999, @pegahcarter, Phasor DEX
/// @notice Gauge contract for distribution of emissions by address
/// @custom:phasor-changes Added time multiplier feature (1x→3x over 90 days)
contract Gauge is IGauge, ERC2771Context, ReentrancyGuardTransient {
    ...
    // ============================================================================
    // PHASOR CUSTOM: Time Multiplier for Staking Loyalty
    ...
    // PHASOR CUSTOM: Track first stake time for time multiplier
    ...
    function earned(address _account) public view returns (uint256) {
        uint256 timeMultiplier = getTimeMultiplier(_account);
        uint256 baseEarned = (balanceOf[_account] * (rewardPerToken() - userRewardPerTokenPaid[_account])) / PRECISION;
        // PHASOR CUSTOM: Apply time multiplier to newly earned rewards
        uint256 multipliedEarned = (baseEarned * timeMultiplier) / PRECISION;
        return multipliedEarned + rewards[_account];
    }

    // ============================================================================
    // PHASOR CUSTOM: Time Multiplier Functions
    // ============================================================================

    /// @notice Get the time-based reward multiplier for an account
    /// @param _account The address to check
    /// @return multiplier The multiplier in 1e18 format (1e18 = 1x, 3e18 = 3x)
    function getTimeMultiplier(address _account) public view returns (uint256) {
        uint256 firstStake = userFirstStakeTime[_account];
        if (firstStake == 0) return TIME_MULTIPLIER_MIN;

        uint256 elapsed = block.timestamp - firstStake;
    ...
    function _depositFor(uint256 _amount, address _recipient) internal nonReentrant {
        ...
        // PHASOR CUSTOM: Track first stake time for time multiplier
        if (userFirstStakeTime[_recipient] == 0) {
            userFirstStakeTime[_recipient] = block.timestamp;
        }
