...
import {IPhasor} from "./interfaces/IPhasor.sol";
...
contract Minter is IMinter {
    using SafeERC20 for IPhasor;

    /// @inheritdoc IMinter
    IPhasor public immutable phasor;
    /// @inheritdoc IMinter
    ...
    constructor(
        ...
        phasor = IPhasor(IVotingEscrow(_ve).token());
        voter = IVoter(_voter);
    ...
    function calculateGrowth(uint256 _minted) public view returns (uint256 _growth) {
        ...
        uint256 _phasorTotal = phasor.totalSupply();
        return (((((_minted * _veTotal) / _phasorTotal) * _veTotal) / _phasorTotal) * _veTotal) / _phasorTotal / 2;
    }
    ...
    function updatePeriod() external returns (uint256 _period) {
        ...
        if (block.timestamp >= _period + WEEK) {
            ...
            uint256 _emission;
            uint256 _totalSupply = phasor.totalSupply();
            ...
            uint256 _balanceOf = phasor.balanceOf(address(this));
            if (_balanceOf < _required) {
                phasor.mint(address(this), _required - _balanceOf);
            }

            phasor.safeTransfer(address(team), _teamEmissions);
            phasor.safeTransfer(address(rewardsDistributor), _growth);
            rewardsDistributor.checkpointToken(); // checkpoint token balance that was just minted in rewards distributor

            phasor.safeIncreaseAllowance(address(voter), _emission);
            voter.notifyRewardAmount(_emission);

            emit Mint(msg.sender, _emission, phasor.totalSupply(), _tail);
        }
    }
}
